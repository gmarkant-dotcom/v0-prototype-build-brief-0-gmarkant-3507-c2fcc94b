...
> | --- | --- | --- |
> | 1 | `app/api/projects/route.ts:105`, a stage label | **DELETED 2026-09-15 by `feat/engagement-ruling`.** It was dead output: `dashboard_workflow_stage` / `dashboard_workflow_label` had no reader anywhere. The classifier, the two fields, the three Sets that fed it and TWO database queries are gone. The document called this surface "where the phrase enters the vocabulary"; the vocabulary now enters in one place only |
...
> | Surface | Unit | State |
> | --- | --- | --- |
> | `app/api/agency/dashboard/route.ts` `workflowStageForProject()` | a project | **LIVE, CUSTOMER-VISIBLE, AND NOW THE ONLY ONE.** Renders the stage pill at `app/agency/dashboard/page.tsx:698`. The other two classifiers were deleted on 2026-09-15; this one is untouched. Its header now says to IMPORT it rather than hand-copy it again |
> | `app/api/agency/active-engagements/route.ts` classifier | a project | **CLASSIFIER DELETED 2026-09-15. THE ROUTE IS LIVE AND WAS KEPT.** >>> The route is fetched by `app/agency/project/page.tsx` and its `projects[].partners` ARE the rows behind that page's whole engagement list. Deleting the route would have broken the page. Only the two dead stage fields went, plus the unread optional declaration of them on `ProjectEngagement` |
> | `app/api/projects/route.ts` `total_active_engagements` | awarded RESPONSE rows on live projects | **DELETED 2026-09-15.** A fifth unit and a real count, reaching the client only inside `agency_dashboard_stats`, which has no reader. It was a NEAR-MISS of the ruled unit: it counted awarded responses, so an assignment with two awarded responses scored 2 here and 1 under Option B. The surviving `agency_dashboard_stats` object is also unread; its other three fields were left as out of scope |
> | `app/api/partner/summary/route.ts` `active_engagements` | awarded `project_assignments` rows, **no liveness at all** | **ROUTE DELETED 2026-09-15.** A sixth unit on a route nothing in this repository fetched. Absence of a fetch is the strongest evidence available and is NOT proof: an external caller would now get a 404. Its auth and role checks went with it, which is safe - a deleted route grants nothing |
