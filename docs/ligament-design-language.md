...
**Type scale (ruled).** 10px is blessed as a named token - it is the most-used size in the product (the mono eyebrow/metadata size) and gets a home: define `--text-micro` (10px) exposed as `text-2xs`. The 9px and 11px arbitrary values migrate into it (S3 batch). The full sanctioned scale:
...
24. Plain `gray-100`/`gray-200`/`gray-300` ruled sanctioned, not a migration gap, for light-portal skeletons, disabled fills, placeholders, neutral badges, and hover overlays - closes the flag raised in the R1 batch. `partner-project-production-detail.tsx` and `request-invitation-modal.tsx` deleted as zero-importer dead code, discovered during the same batch. `lead-agency-filter.tsx`'s loading skeleton and clear-filter button, found using the dark portal's `bg-white/5`/`text-foreground` directly on the light page, moved onto the sanctioned gray/vendor-foreground treatment
