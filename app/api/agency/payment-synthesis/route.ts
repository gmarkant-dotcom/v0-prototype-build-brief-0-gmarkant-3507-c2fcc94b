...
export async function POST(req: Request) {
  try {
    ...
    const partnerContexts = Array.from(partnerContextMap.values())
    ...
    const summary = [
      `Project: ${projectName}`,
      `Client: ${clientName}`,
      `Client budget (projects.budget_range): ${clientBudgetRaw || "N/A"}${parsedBudget != null ? ` (parsed upper estimate: ${parsedBudget})` : ""}`,
      "",
    ...
    const systemPrompt = `You are a cash flow protection advisor for a lead creative agency. Your job is to analyze partner payment obligations and recommend a unified payment schedule that protects the lead agency's margin and minimizes cash flow risk.
...
Prioritize: paying partners after the agency receives client funds, flagging any deposit requirements that create cash flow gaps, and ensuring total partner payments do not exceed client budget minus a reasonable margin buffer.`
    ...
    const arr = extractJsonArray(ai.text || "")
    if (!arr) {
      return NextResponse.json(
        { error: "AI returned non-JSON output", detail: (ai.text || "").slice(0, 500) },
