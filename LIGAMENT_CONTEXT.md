...
| P10 | Wire COI upload to `coi_on_file` | `/partner/legal` "Required Documents" Certificate of Insurance (COI) row only uploads a file via `/api/upload` and updates local React state - it never persists document status and never drives `profiles.business_criteria.insurance.coi_on_file`. Needs a real documents table (or reuse of an existing one) plus wiring the COI row's completion to set `coi_on_file: true`. |
...
| P12 | Dual-role base-role bug in API routes | ~15 `app/api/partner/*` routes (rate-info, payments, projects, rfps, blob-download, onboarding/file, summary, onboarding-packages/[id], etc.) and `contexts/paid-user-context.tsx:150` gate on `profile.role === "partner"` only, not `active_role`. Per `047_dual_role_profiles.sql` + `switch-role/route.ts`, a dual-role account's base `role` never becomes `"partner"` - only `active_role` flips. This likely blocks dual-role users from real partner actions (bid submit, payments, downloads) server-side, not just navigation. Needs its own review pass, not a blind find/replace, since these are server-side authorization checks. |
