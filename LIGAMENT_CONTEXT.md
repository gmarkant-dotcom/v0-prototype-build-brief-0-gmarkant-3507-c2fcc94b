...
| 064 | Bid Management rebuild Phase 1: added ai_summary_short, ai_summary_detailed, ai_summary_generated_at to partner_rfp_responses; created bid_decompositions (per-bid AI cost breakdown, unique on response_id) and bid_comparisons (cached multi-bid comparison narrative, unique on agency_id + response_ids_hash). RLS on both new tables matches client_cash_flow's pattern - single FOR ALL policy, agency_id = auth.uid(). WRITTEN, NOT YET APPLIED - run in Supabase SQL Editor before deploying the /api/agency/bids/[responseId]/generate-summary, /decompose, and /api/agency/bids/compare endpoints. |
| 065 | Bid Management Phase 2 (scoring framework): created bid_scoring_criteria, bid_scoring_templates, bid_evaluations (unique on response_id), bid_evaluation_scores (unique on evaluation_id + criterion_id); added composite_score numeric to partner_rfp_responses. RLS on the first three tables matches 064's pattern (agency_id = auth.uid()); bid_evaluation_scores has no agency_id column per spec, so its policy reaches through an EXISTS subquery against bid_evaluations.agency_id instead. WRITTEN, NOT YET APPLIED - run in Supabase SQL Editor before deploying the /api/agency/scoring/* and /api/agency/bids/[responseId]/ai-score, /evaluation, /api/agency/bids/rank endpoints. |
| 066 | Bid Management Phase 3 (Delivery Performance): created delivery_reviews (unique on project_id + partnership_id, links to project_assignments and partner_rfp_responses, scored against the same bid_scoring_criteria) and delivery_review_scores (unique on review_id + criterion_id); added reliability_summary text, reliability_summary_generated_at timestamptz to partnerships. RLS matches 065's bid_evaluations/bid_evaluation_scores pattern exactly, plus an added partner-facing SELECT policy - partners may read their own delivery_reviews rows where status = 'complete', reached through partnerships.partner_id. WRITTEN, NOT YET APPLIED - run in Supabase SQL Editor before deploying /api/agency/delivery-reviews, /api/agency/pool/[partnerId]/performance, and the Performance Scores section on app/partner/projects/page.tsx. |
| 068 | Partner import (spreadsheet + manual Add Partner fix): added contact_name text, company_name text, phone text, website text to partnerships - ghost/unclaimed (Discovered) rows have no profiles row to join, so these four columns are the only place to persist that data pre-claim. Discipline/type and any unmapped import fields deliberately have no dedicated column; they're namespaced into the existing partnership_notes jsonb instead, alongside the {blacklisted} flag already stored there. No RLS changes - existing partnerships policies are row-level. WRITTEN, NOT YET APPLIED - run in Supabase SQL Editor before deploying the spreadsheet import route, the fixed Add Partner API route, or the updated Discovered row renderer on app/agency/pool/page.tsx. |
...
| P8 | Numeric insurance-limit parser - DONE | Implemented in `lib/insurance-limit-parser.ts` (`parseInsuranceLimit`, `meetsInsuranceMinimum`), tested in `lib/insurance-limit-parser.test.ts` (run via `npx tsx`). Wired into the `/agency/bids` Business Criteria section: a held limit that fails the required minimum now upgrades the badge to Missing; an unparseable limit on either side keeps the old presence-only Met badge with a "Limit not auto-verified" hint rather than guessing. |
...
| P10 | Wire COI upload to `coi_on_file` - DONE for COI | `/partner/legal` COI row now uploads via `/api/upload`, then saves the returned URL to `profiles.business_criteria.insurance.coi_document_url` and sets `coi_on_file: true` in the same action (no new migration - added as a JSONB key via `lib/business-criteria.ts`). Page load restores "complete" status from persisted state instead of defaulting to local-only. EIN Verification and W-9 Form rows on the same checklist are still local-only status (not fixed in this pass, by design - see original note). |
| P11 | Partnership Context accordion | `/partner/profile` "Partnership Context" section uses a single dropdown to select one partnership at a time for editing. Replace with an accordion showing all partnerships with expand/collapse, so partners can see and edit multiple partnership contexts without switching the dropdown back and forth. |
| P12 | Dual-role base-role bug in API routes - DONE for (a)/(b) | Audited every `role === "partner"/"agency"` (and negated) check across `app/api/**` and `contexts/paid-user-context.tsx`. Fixed ~31 partner- and agency-side authorization gates to accept `role === "x" || active_role === "x"`, plus `checkFeatureAccess()` in `paid-user-context.tsx` (added `activeRole` state so a dual-role user active as partner never sees the upgrade paywall). Left 4 billing-coupled routes unchanged - see P13. |
| P13 | Billing-coupled AI routes: dual-role intent unclear | `app/api/agency/msa/ai-schedule/route.ts`, `app/api/agency/payment-synthesis/route.ts`, `app/api/ai/master-brief/route.ts`, `app/api/documents/extract-text/route.ts` all gate AI feature access on `profile.role` combined with `is_paid`/`is_admin`. Deliberately left on base-role-only checks during the P12 audit - unclear whether a dual-role user's own billing status or their currently-active portal should govern access to a paid AI feature. Needs a product decision, not a blind pattern-match fix. |
| P14 | Missing bid action timestamp columns | Bid activity timeline (`lib/bid-timeline.ts`) only shows RFP sent / Bid submitted / Feedback received / Awarded because those are the only action timestamps that exist. `shortlisted_at`, `meeting_requested_at`, and `declined_at` don't exist as columns - `partner_rfp_responses.status` only tracks current state, not when it changed. Would need: `ALTER TABLE partner_rfp_responses ADD COLUMN IF NOT EXISTS shortlisted_at timestamptz, ADD COLUMN IF NOT EXISTS meeting_requested_at timestamptz, ADD COLUMN IF NOT EXISTS declined_at timestamptz;` plus setting them in the PATCH handler (`app/api/agency/rfp-responses/[id]/route.ts`) wherever `status` transitions to each value. Not migrated in this pass per instruction. |
| P15 | "Awarded" timeline entry unavailable for guest bids | `project_assignments.awarded_at` is keyed by `(project_id, partnership_id)`, but `rfp_magic_tokens` (guest/magic-link bids) has no `partnership_id` at all - there is no key to join against. The guest response page's Status & Feedback timeline will never show an "Awarded" entry even when a guest bid is awarded. Would need either a `partnership_id` backfill path for guest-to-partner conversions, or a denormalized `awarded_at` directly on `partner_rfp_responses`. |
| P16 | New env var: `WEBHOOK_SECRET` | Added to protect `/api/admin/notify-new-user` (security fix - route previously had no auth and would mint a real grant-access token for any POSTed `{record:{id,email}}`). Must be set in Vercel and on the Supabase DB webhook that calls this route, sent as the `x-webhook-secret` request header. Until set, the route returns 401 for all callers including the legitimate webhook. |
