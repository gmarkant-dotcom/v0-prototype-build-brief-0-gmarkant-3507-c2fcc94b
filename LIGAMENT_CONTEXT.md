...
> 078's header measured 15 accounts on 2026-08-17, 7 of which chose `partner` at signup and
> carried `role='agency'`, and wrote per-account `UPDATE` statements for them into
> `docs/m1-prework-report.md` Item 1. Live query D4 on 2026-08-20 returns **18 accounts, every
> one of them with `profiles.role` matching its signup choice, zero mismatches.** There is
> nothing left to correct, and the statements in that report must not be run - re-read the
> live data first if this is ever reopened. The forward defect was already closed at the
